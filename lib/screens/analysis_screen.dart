...
class _AnalysisScreenState extends State<AnalysisScreen> {
  ...
  Widget _buildCategoryBreakdown(List<Map<String, dynamic>> data, AppColors semantic) {
    if (data.isEmpty) {
      return const Text("No spending data yet.", style: TextStyle(color: Colors.grey));
    }
    
    // Find max value for progress bar calculation
    final maxVal = data.fold<int>(0, (prev, e) => (e['total'] as int) > prev ? (e['total'] as int) : prev);
            ...
            children: [
               Row(
                 ...
                 children: [
                   ...
                   Text(CurrencyHelper.format(total), style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 13)),
                 ],
               ),
                       ...
                       Container(
                         ...
                         decoration: BoxDecoration(
                           gradient: LinearGradient(
                             colors: [semantic.expense, semantic.expense.withValues(alpha: 0.7)],
                           ),
                           borderRadius: BorderRadius.circular(4),
                           boxShadow: [
                             BoxShadow(
  ...
  Widget _buildInsightCard(List<Map<String, dynamic>> data, AppColors semantic) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: isIncrease 
             ? [semantic.overspent.withValues(alpha: 0.2), semantic.overspent.withValues(alpha: 0.05)]
             : [semantic.income.withValues(alpha: 0.2), semantic.income.withValues(alpha: 0.05)],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight
        ),
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: isIncrease ? semantic.overspent.withValues(alpha: 0.3) : semantic.income.withValues(alpha: 0.3)
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              Icon(isIncrease ? Icons.warning_amber_rounded : Icons.thumb_up_alt_outlined, 
